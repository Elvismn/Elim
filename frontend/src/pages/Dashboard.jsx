...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const upcomingMaintenance = await dashboardService.getUpcomingMaintenance({ days: 30 })
      
      const unverifiedFuel = await dashboardService.getUnverifiedFuelRecords({ limit: 5 })
      ...
      if (expiringDocs?.data?.documents) {
        expiringDocs.data.documents.slice(0, 3).forEach(d => {
          activities.push({
            ...
            user: 'System',
            time: `${d.daysUntilExpiry || 0} days left`
          })
        })
      }
  ...
  const statCards = [
    { title: 'Total Students', value: stats.totalStudents, icon: <Users className="h-6 w-6" />, color: 'from-blue-500 to-blue-600', onClick: () => handleNavigation('/students') },
    { title: 'Total Parents', value: stats.totalParents, icon: <GraduationCap className="h-6 w-6" />, color: 'from-purple-500 to-purple-600', onClick: () => handleNavigation('/parents') },
    { title: 'Staff Members', value: stats.totalStaff, icon: <Briefcase className="h-6 w-6" />, color: 'from-indigo-500 to-indigo-600', onClick: () => handleNavigation('/staff') },
    { title: 'School Vehicles', value: stats.totalVehicles, icon: <Car className="h-6 w-6" />, color: 'from-green-500 to-emerald-600', onClick: () => handleNavigation('/vehicles') }
  ]

  const secondaryStatCards = [
    { title: 'Departments', value: stats.totalDepartments, icon: <Building2 className="h-6 w-6" />, color: 'from-yellow-500 to-amber-600', onClick: () => handleNavigation('/departments') },
    { title: 'Stakeholders', value: stats.totalStakeholders, icon: <HeartHandshake className="h-6 w-6" />, color: 'from-pink-500 to-rose-600', onClick: () => handleNavigation('/stakeholders') },
    { title: 'Active Vehicles', value: stats.activeVehicles, icon: <Truck className="h-6 w-6" />, color: 'from-cyan-500 to-sky-600', onClick: () => handleNavigation('/vehicles') },
    { title: 'Total Fuel Cost', value: formatCurrency(stats.totalFuelCost), icon: <Fuel className="h-6 w-6" />, color: 'from-orange-500 to-amber-600', onClick: () => handleNavigation('/fuel-records') }
  ]

  const alertCards = [
    { title: 'Vehicles in Maintenance', value: stats.vehiclesInMaintenance, icon: <Wrench className="h-6 w-6" />, color: 'from-orange-500 to-red-600', description: 'Currently in workshop', onClick: () => handleNavigation('/maintenance') },
    { title: 'Upcoming Maintenance', value: stats.upcomingMaintenance, icon: <Calendar className="h-6 w-6" />, color: 'from-yellow-500 to-orange-600', description: 'Due within 30 days', onClick: () => handleNavigation('/maintenance') },
    { title: 'Expiring Documents', value: stats.expiringDocuments, icon: <FileText className="h-6 w-6" />, color: 'from-red-500 to-pink-600', description: 'Need attention', onClick: () => handleNavigation('/vehicle-documents') },
    { title: 'Unverified Fuel Records', value: stats.unverifiedFuelRecords, icon: <FileCheck className="h-6 w-6" />, color: 'from-purple-500 to-violet-600', description: 'Awaiting verification', onClick: () => handleNavigation('/fuel-records') }
  ]
  ...
  const quickActions = [
    ...
    { label: 'Fuel Records', icon: <Fuel className="w-5 h-5" />, action: 'fuel-records', color: 'amber' }
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {statCards.map((stat, index) => (
          <button key={index} onClick={stat.onClick} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all text-left w-full">
            <div className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">{stat.title}</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">
                    {loading ? <span className="inline-block w-12 h-6 bg-gray-200 animate-pulse rounded"></span> : stat.value}
                  </p>
                </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {alertCards.map((alert, index) => (
          <button key={index} onClick={alert.onClick} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all text-left w-full">
            <div className="p-5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-gray-900">
                  {loading ? <span className="inline-block w-8 h-6 bg-gray-200 animate-pulse rounded"></span> : alert.value}
                </span>
              </div>
      ...
      <Card title="Quick Actions" subtitle="Navigate to different sections">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {quickActions.map((action, index) => (
            <button key={index} onClick={() => handleQuickAction(action.action)} className="flex flex-col items-center justify-center p-4 border border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition-all duration-200 group">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-center justify-between py-3 border-b last:border-0 hover:bg-gray-50 px-2 rounded transition-colors">
